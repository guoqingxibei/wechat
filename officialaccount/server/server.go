...
type Server struct {
	*context.Context
	...
	parseXmlToMsg  func(rawXMLMsgBytes []byte, msg *message.MixMessage) error
...
// NewServer init
func NewServer(context *context.Context, parseXmlToMsg func(rawXMLMsgBytes []byte, msg *message.MixMessage) error) *Server {
	srv := new(Server)
	srv.Context = context
	srv.parseXmlToMsg = parseXmlToMsg
	return srv
}
...
func (srv *Server) parseRequestMessage(rawXMLMsgBytes []byte) (msg *message.MixMessage, err error) {
	...
	if !srv.isJSONContent {
		err = srv.parseXmlToMsg(rawXMLMsgBytes, msg)
		return
	}
	// parse json
